...
async function jsonParser(url: string, parser: (json: string) => void) {
...
const subreddit = (sub: string, topic: string, a:number) => async () =>{

  const response = await axios.get(`https://www.reddit.com/r/${sub}/top.json?t=week`);

  Object.entries(response.data.data.children).forEach(([key, value]) => {
